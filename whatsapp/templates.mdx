...
Special templates to request permission for WhatsApp voice calls. Enabling calling on a sender can create one for you.

**Use for:**
- Asking a customer for permission to call them on WhatsApp
- Must be sent inside an open 24-hour chat (they messaged you first)

**Approval:** Automatic (when using the standard format)

<Warning>
WhatsApp will not deliver this template to a brand-new number. Send a regular template first, wait for their reply, then ask to call. Full flow: [WhatsApp Calling](/whatsapp/calling).
</Warning>
