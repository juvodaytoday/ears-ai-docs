...
By default, when your AI assistant replies, the incoming message is marked as read and the customer sees a "typing…" indicator before the reply. If your team also handles chats from the WhatsApp Business app, this makes it hard to tell which conversations still need a person.

Enable **Keep conversations unread** to leave incoming messages marked as unread, so your team can spot the chats that still need attention. A team member marks a conversation as read by opening it in the WhatsApp Business app.

1. Edit your WhatsApp sender
2. Enable the **Keep conversations unread** toggle
3. Save changes

<Note>
With this enabled, customers no longer see the "typing…" indicator before an AI reply — the typing indicator and the read status are linked, so keeping messages unread also turns the indicator off.
</Note>
...
After the sender is online, has an assistant, and a messaging limit of at least 2,000 / 24hr, click **Enable Calling**. Customers can then call this number, and you can call them after they grant permission.

Calling is not available on Coexistence (WhatsApp Business app). Use a Dedicated Platform Number or an External Mobile Number.

Read the full flow, limits, and FAQ: [WhatsApp Calling](/whatsapp/calling).
